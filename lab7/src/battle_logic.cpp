...
void BattleLogic::visit_attack(Knight& self, std::shared_ptr<NPC> other) {
...
void BattleLogic::visit_attack(Dragon& self, std::shared_ptr<NPC> other) {
    // if (!self.is_alive() || !other->is_alive()) return;


    // other->set_dead();
    // safe_cout(cout_mutex_, self.get_name() + " (Dragon) KILLED " + 
    //           other->get_name() + " (TEST OVERRIDE).");
    perform_attack(self.shared_from_this(), other);
}